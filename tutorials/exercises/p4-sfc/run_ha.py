...
before = [-1,-1,-1,-1]
chain1 = [2,4,0,0]
chain1_len = len(list([x for x in chain1 if (x > 0)]))
chain2 = [3,4,0,0]
chain2_len = len(list([x for x in chain2 if (x > 0)]))
...
def writeClassifierRules(p4info_helper,sw,dscp,id,sc,sf1,sf2,sf3,sf4,update):
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        while True:
            ...
            bb = list([num for num in before if num >= 0])
